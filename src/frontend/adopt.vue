<template>
  <div class="adopt">
    <loading :active.sync="isLoading">
      <div class="loadingio-spinner-spin-vr67c069ls">
        <div class="ldio-i4ihhev39wf">
          <div><div></div></div>
          <div><div></div></div>
          <div><div></div></div>
          <div><div></div></div>
          <div><div></div></div>
          <div><div></div></div>
          <div><div></div></div>
        </div>
      </div>
    </loading>
    <alert />
    <div class="banner">
      <div>
        <h4>愛要及時</h4>
        <p>快把我帶回家吧 <span>。。。</span></p>
      </div>
    </div>
    <div class="adopt-item">
      <div class="adopt-item-category">
        <h5>分類</h5>
        <div class="adopt-item-category-list">
          <ul>
            <li>
              <a href="#" @click.prevent="categorylist('all')"
                ><img src="~@/assets/images/adopt/all.png" alt="" /> 全部</a
              >
            </li>
            <li v-for="(item, key) in categoryitem" :key="key">
              <a href="#" @click.prevent="categorylist(item)"
                ><img src="~@/assets/images/adopt/animal-shelter.png" alt="" />
                {{ item }}</a
              >
            </li>
          </ul>
        </div>
      </div>
      <div class="adopt-item-wrap">
        <h5>寵物</h5>
        <div class="adopt-item-content">
          <div
            class="adopt-item-content-list"
            v-for="(item, key) in filterData"
            :key="key"
          >
            <div class="header">點閱次數</div>
            <div class="content">
              <div class="addfav"></div>
              <div class="addAdopt">
                <button @click="editfav(item)" title="加入關注">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    height="36"
                    viewBox="0 0 60 55"
                    width="36"
                  >
                    <g id="Page-1" fill="none" fill-rule="evenodd">
                      <g
                        id="029---Pet-Health"
                        fill="rgb(0,0,0)"
                        fill-rule="nonzero"
                        class="addAdopted"
                        :class="{
                          adoptadded: favorite.indexOf(item.id) !== -1
                        }"
                      >
                        <path
                          id="Shape"
                          d="m44.5 1c-2.1117095.00235057-4.2098296.3378068-6.217.994-1.0701203-1.22169039-2.5996775-1.94391085-4.223-1.994-3.0113285.16336959-5.9902609.70392472-8.867 1.609-.6713244.19637636-1.2935809.53238126-1.826.986-2.4884178-1.0552075-5.1640981-1.59769002-7.867-1.595-8.691 0-15.5 8.4-15.5 19.12 0 11.256 7.278 26.691 27.588 34.106.019.009.039.015.058.022.6666667.244 1.3486667.4773333 2.046.7.0993529.0331844.2032594.0507271.308.052.1049487.0001819.2092562-.0163588.309-.049 1.8881305-.6122298 3.7432824-1.3218471 5.558-2.126.025-.009.051-.005.076-.016s.041-.032.065-.044c17.58-7.865 23.992-22.097 23.992-32.645 0-10.72-6.809-19.12-15.5-19.12zm13.5 19.12c-.1176303 6.0689625-2.0472791 11.9641248-5.541 16.928-2.591-5.752-5.9-10.969-7.692-12.755-1.3044537-1.4498544-2.9648945-2.5339495-4.817-3.145l-.412-2.889c1.7089365-1.1858634 2.6671558-3.1839902 2.522-5.259.0305788-3.27343787-.8275725-6.4938381-2.483-9.318 1.6019484-.45207297 3.2584855-.68155871 4.923-.682 7.57 0 13.5 7.52 13.5 17.12zm-56 0c0-9.6 5.93-17.12 13.5-17.12 2.236619.00392316 4.4531085.42267252 6.537 1.235l-1.951 2.923c-.4143229.62131721-1.0442246 1.0670225-1.768 1.251l-5.807 1.442c-.8588936.2173243-1.4704984.9765297-1.5 1.862-.064 1.7.163 7.375 5.732 9.235.102174.034303.2092215.0518628.317.052 2.949389.0339263 5.8751075.5313322 8.67 1.474l-.244.526c-1.6078346 3.0898276-2.4598228 6.5169724-2.486 10 .0667239 3.8272001 1.4834204 7.5077775 4 10.392v8.465c-18.368-7.228-25-21.373-25-31.737zm11-8.329 2-.5v1.709h-1.968c-.0340179-.4020432-.0447025-.8057202-.032-1.209zm23.156 38.7c-.1048519-.4900418-.1571476-.9898677-.156-1.491-.0521315-2.4835335 1.0677904-4.8467428 3.023-6.379 1.6567991-1.1541931 3.7800313-1.4062355 5.661-.672.3394023.1128959.7132522.0361471.9807241-.201336.2674719-.237483.3879304-.599621.316-.95s-.3253218-.6357681-.6647241-.748664c-2.4920741-.9320098-5.2833511-.5762735-7.462.951-2.4732452 1.9105965-3.901389 4.875088-3.854 8-.0038899.7829213.0970055 1.5628425.3 2.319-1.368.582-2.8 1.13-4.305 1.629-.34-.112-.666-.232-1-.349v-9.599c.0002093-.2645618-.1044287-.5184257-.291-.706-2.3044227-2.5565821-3.6182449-5.8531975-3.704-9.294.0252145-3.1914691.8138449-6.3305613 2.3-9.155l.244-.529c.2294894-.4891242.2475721-1.0511351.05-1.554-.2045596-.5119941-.6106421-.9173534-1.123-1.121-2.970452-1.0438215-6.0907777-1.5980432-9.239-1.641-1.8466721-.6326495-3.2694225-2.1251756-3.813-4h1.581c1.1045695 0 2-.8954305 2-2v-2.2l1.809-.449c1.2049818-.3061856 2.2535946-1.04837402 2.943-2.083l2.337-3.5c.0785063-.11655599.1650498-.22749206.259-.332.3705756-.42819561.8566011-.74064056 1.4-.9 2.6985911-.84530503 5.4895317-1.3610515 8.312-1.536 1.2567393.10358472 2.4046699.7535913 3.14 1.778 1.9248249 2.68418003 2.9282534 5.91971066 2.86 9.222 0 1.914-.682 3.2-2.029 3.812-1.4227316.5432205-3.0012185.5102158-4.4-.092-.381114-.1513089-.631234-.5199486-.631-.93v-6.79c0-.55228475-.4477153-1-1-1s-1 .44771525-1 1v6.79c.0045439 1.2285382.7530983 2.3317959 1.893 2.79 1.4995833.6112872 3.1455704.7691406 4.734.454l.344 2.4c.1126488.7888437.6865411 1.4336355 1.457 1.637 1.5120964.5283904 2.863694 1.4351932 3.926 2.634 1.553 1.544 5.05 6.934 7.737 13.2-4.0063823 4.9727493-9.1206904 8.9391291-14.934 11.582z"
                        />
                      </g>
                    </g>
                  </svg>
                </button>
              </div>
              <img :src="item.image" alt="" />
              <div class="content-info">
                <div class="content-info-name">動物品種 : {{ item.title }}</div>
                <div class="content-info-category">
                  類型 : {{ item.category }}
                </div>
                <div class="content-info-gender">性別 : {{ item.unit }}</div>
                <div class="content-info-more">
                  <a
                    href=""
                    class="color-main adopt-more"
                    @click.prevent="learnmore(item.id)"
                    >了解更多</a
                  >
                  <a
                    href=""
                    class="color-main adopt-more ml-2"
                    @click.prevent="addCart(item.id)"
                    >預約領養</a
                  >
                </div>
              </div>
            </div>
          </div>
        </div>
        <page
          :pages="pagination"
          @emitPage="products"
          class="d-flex m-auto mt-3 justify-content-center"
        ></page>
      </div>
    </div>
  </div>
</template>
<script>
import page from '@/components/pagination'
import alert from '@/components/AlertMessage'

export default {
  name: 'adopt',
  components: {
    page,
    alert
  },
  data () {
    return {
      data: [],
      newData: [],
      adopt: [],
      category: 'all',
      newCategory: new Set(),
      // 新增loaclstorage
      favorite: JSON.parse(localStorage.getItem('addfav')) || [],
      favoriteadd: false,
      adoptVal: 0,
      adoptId: [],
      pagination: [],
      isLoading: false
    }
  },
  methods: {
    products: function (page = 1) {
      // 取得產品
      const url = `${process.env.VUE_APP_API}/api/${process.env.VUE_APP_CUSTOMPATH}/products?page=${page}`
      const vm = this
      vm.isLoading = true
      vm.$http.get(url).then(function (res) {
        vm.isLoading = false
        vm.data = res.data.products
        vm.pagination = res.data.pagination
      })
    },
    categorylist: function (list) {
      // 列表取得與產品過濾
      const vm = this
      vm.category = list
      vm.products()
    },
    editfav: function (item) {
      // 新增最愛
      const vm = this
      const num = vm.favorite.indexOf(item.id)
      if (num === -1) {
        vm.favorite.push(item.id)
        vm.isLoading = true
        vm.favoriteadd = true
        vm.$bus.$emit('message', '關注成功', 'success')
      } else {
        vm.favorite.splice(num, 1)
        vm.favoriteadd = false
        vm.$bus.$emit('message', '取消關注', 'error')
      }
      localStorage.setItem('addfav', JSON.stringify(vm.favorite))
      vm.isLoading = false
      vm.$bus.$emit('favnum', vm.favorite.length)
    },
    learnmore: function (id) {
      // 取得單一品項
      const url = `${process.env.VUE_APP_API}/api/${process.env.VUE_APP_CUSTOMPATH}/product/${id}`
      const vm = this
      vm.$http.get(url).then(function (res) {
        vm.adopt = res.data.product.id
        vm.$router.push(`/adopt/${res.data.product.id}`)
      })
    },
    addCart: function (id, qty = 1) {
      // 預約領養
      const url = `${process.env.VUE_APP_API}/api/${process.env.VUE_APP_CUSTOMPATH}/cart`
      const vm = this
      vm.isLoading = true
      const cart = {
        product_id: id,
        qty: qty
      }
      if (vm.adoptId.includes(id)) {
        vm.$bus.$emit('message', '已經加入預約領養', 'error')
        vm.isLoading = false
      } else {
        vm.$http.post(url, { data: cart }).then(function (res) {
          if (res.data.success) {
            vm.isLoading = false
            vm.getCart()
            vm.$bus.$emit('message', '已加入預約領養', 'success')
            console.log(res)
          } else {
            console.log('error')
            vm.isLoading = false
          }
        })
      }
    },
    getCart: function () {
      const url = `${process.env.VUE_APP_API}/api/${process.env.VUE_APP_CUSTOMPATH}/cart`
      const vm = this
      vm.isLoading = true
      vm.$http.get(url).then(function (res) {
        vm.isLoading = false
        vm.adoptVal = res.data.data.carts.length
        vm.adoptId = res.data.data.carts.map(function (item) {
          return item.product_id
        })
        vm.$bus.$emit('cartVal', vm.adoptVal)
      })
    }
  },
  computed: {
    filterData: function () {
      // 產品過濾
      const vm = this
      if (vm.category !== 'all') {
        const filteradopt = vm.data.filter(function (item) {
          return vm.category === item.category
        })
        vm.data = filteradopt
      }
      return vm.data
    },
    categoryitem: function () {
      // 列表取得
      const vm = this
      vm.data.forEach(function (item) {
        vm.newCategory.add(item.category)
      })
      return vm.newCategory
    }
  },
  created () {
    const vm = this
    vm.products()
    vm.getCart()
  }
}
</script>
