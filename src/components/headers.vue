    ...
    <div class="header">
      ...
      <div class="header-list">
        <ul>
          ...
          <li class="header-list-4">
            <router-link to="/login" class="color-link-main"
              >登入會員</router-link
            >
          </li>
          <li class="header-list-5">
            <router-link to="/adopt_cart" class="color-link-main"
              >預約領養</router-link
            >
          </li>
          <li class="header-list-6">
            <router-link to="/adopt/favorite" class="color-link-main"
              >關注汪汪
              <p v-if="facnumber > 0">{{ facnumber }}</p>
            </router-link>
          </li>
        </ul>
      </div>
...
export default {
  ...
  created () {
    const vm = this
    vm.facnumber = vm.favorite.length
    vm.$bus.$on('favnum', val => {
      vm.facnumber = val
    })
  }
}
...
<style scoped>
.header-list-1 a::before {
  content: url(../assets/images/header/Adoptdog.png);
}
.header-list-2 a::before {
  content: url(../assets/images/header/savegod.png);
}
.header-list-3 a::before {
  content: url(../assets/images/header/purpose.png);
}
.header-list-4 a::before {
  content: url(../assets/images/header/login.png);
}
.header-list-5 a::before {
  content: url(../assets/images/header/cart.png);
}
.header-list-6 a::before {
  content: url(../assets/images/header/favorite.png);
}
